...
namespace CalamityEntropy.Projectiles
{
    public class VoidMark: ModProjectile
    {
        ...
        public override bool PreDraw(ref Color lightColor)
        {
            ...
            for (int i = 0; i < 3; i++)
            {
                flame1.Add(ModContent.Request<Texture2D>("CalamityEntropy/Projectiles/VM/flame" + i.ToString()).Value);
            }
            for (int i = 1; i < 6; i++)
            {
                flame2.Add(ModContent.Request<Texture2D>("CalamityEntropy/Projectiles/VM/darkflame" + i.ToString()).Value);
            }
            ...
            Texture2D tx = ModContent.Request<Texture2D>("CalamityEntropy/Projectiles/VoidMark").Value;
            float x = 0f;
            if (odp.Count > 16)
            {
            ...
            Texture2D eye = ModContent.Request<Texture2D>("CalamityEntropy/Projectiles/VM/Eye").Value;
