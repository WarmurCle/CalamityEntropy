    ...
    public class IceEdge : ModProjectile
    {
        ...
        public override bool PreDraw(ref Color lightColor)
        {
            if (exps > 0)
            {
                if (htd)
                {
                    Texture2D tx = ModContent.Request<Texture2D>("CalamityEntropy/Projectiles/POP/exp" + frame.ToString()).Value;
            ...
            {
                Texture2D tx = ModContent.Request<Texture2D>("CalamityEntropy/Projectiles/IceEdge").Value;
                float x = 0f;
