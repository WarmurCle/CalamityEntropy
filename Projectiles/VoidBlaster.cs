...
namespace CalamityEntropy.Projectiles
{
    public class VoidBlaster : ModProjectile
    {
        ...
        public override void AI()
        {
            ...
            if (Projectile.ai[0] > 0 && Projectile.ai[0] % 3 == 0)
            {
                ...
                if (Projectile.ai[0] > 200)
                {
                    ...
                    {
                        Projectile.Kill();
                        SoundEngine.PlaySound(new("CalamityEntropy/Sounds/vbdisapear"), Projectile.Center);
                    }
                }
            }
            if (!sspl)
            {
                SoundEngine.PlaySound(new("CalamityEntropy/Sounds/vbapear"), Projectile.Center);
                sspl = true;
            }
            if (Projectile.ai[0] == 14)
            {
                ...
                {
                    SoundEngine.PlaySound(new("CalamityEntropy/Sounds/laser"), Projectile.Center);
                }
        ...
        public override bool PreDraw(ref Color lightColor)
        {
            ...
            Texture2D tx = ModContent.Request<Texture2D>("CalamityEntropy/Projectiles/VB/VoidBlaster" + frame.ToString()).Value;
            ...
            if (frame == 8)
            {
                tx = ModContent.Request<Texture2D>("CalamityEntropy/Projectiles/VB/VoidBlaster" + ((ct / 3) % 3 + 6).ToString()).Value;
            }
