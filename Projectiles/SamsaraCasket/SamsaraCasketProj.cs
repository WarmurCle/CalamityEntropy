    ...
    public class SamsaraCasketProj : ModProjectile
    {
        ...
        public override bool PreDraw(ref Color lightColor)
        {
            ...
            if (spawnAnm > 0) {
                ...
                Texture2D back_ = ModContent.Request<Texture2D>("CalamityEntropy/Projectiles/SamsaraCasket/a" + tex).Value;
                Texture2D front_ = ModContent.Request<Texture2D>("CalamityEntropy/Projectiles/SamsaraCasket/c" + tex).Value;
                ...
                for (int i = 0; i < 6; i++)
                {
                    ...
                    {
                        middleTex_ = ModContent.Request<Texture2D>("CalamityEntropy/Projectiles/SamsaraCasket/b" + i.ToString()).Value;
                        bool glow = false;
                        if (swords[i])
                        {
            ...
            Texture2D back = ModContent.Request<Texture2D>("CalamityEntropy/Projectiles/SamsaraCasket/a" + tex).Value;
            Texture2D front = ModContent.Request<Texture2D>("CalamityEntropy/Projectiles/SamsaraCasket/c" + tex).Value;
            Texture2D middleTex;
            ...
            for (int i = 0; i < 6; i++)
            {
                ...
                if (i <= modPlayer.sCasketLevel)
                {
                    middleTex = ModContent.Request<Texture2D>("CalamityEntropy/Projectiles/SamsaraCasket/b" + i.ToString()).Value;
                    bool glow = false;
                    if (swords[i])
                    {
