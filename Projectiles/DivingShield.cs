...
namespace CalamityEntropy.Projectiles
{
    public class DivingShield : ModProjectile
    {
        ...
        public override void AI(){
            ...
            if (!pld)
            {
                SoundStyle sd = new SoundStyle("CalamityEntropy/Sounds/divine_intervention");
                sd.Volume = 0.36f;
            ...
            if (Projectile.timeLeft > 14)
            {
                foreach (Projectile p in Main.projectile)
                {
                    if (p.hostile && p.Colliding(p.Hitbox, Projectile.Hitbox) && p.active)
                    {
                        ...
                        lsj = 0.36f;
                        SoundStyle sd = new SoundStyle("CalamityEntropy/Sounds/shield");
                        sd.Volume = 0.4f;
                        SoundEngine.PlaySound(sd, Projectile.Center);
        ...
        public override bool PreDraw(ref Color lightColor)
        {
            ...
            tx = ModContent.Request<Texture2D>("CalamityEntropy/Projectiles/DivingShield").Value;
