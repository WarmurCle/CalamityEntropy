...
namespace CalamityEntropy.Content.Particles
{
    public class CosmicMetaball : Metaball
    {
        ...
        public override Color EdgeColor => new(40, 17, 140);
...
*/
            layerAssets.Add(ModContent.Request<Texture2D>("CalamityEntropy/Assets/Extra/planetarium_blue_base", AssetRequestMode.ImmediateLoad));
            for (int i = 1; i <= 5; i++)
            {
                if (i == 2) continue;
                layerAssets.Add(ModContent.Request<Texture2D>("CalamityEntropy/Assets/Extra/planetarium_starfield_" + i.ToString(), AssetRequestMode.ImmediateLoad));
            }
        }
        ...
        public override Vector2 CalculateManualOffsetForLayer(int layerIndex)
        {
            switch (layerIndex)
            {
                ...
                case 4:
                    ...
                    return offset;
                case 5:
                    offset = Vector2.UnitX * Main.GlobalTimeWrappedHourly * 0.033f + (Main.GlobalTimeWrappedHourly * 1.77f).ToRotationVector2() * 0.04725f;
                    offset.Y += CalamityUtils.PerlinNoise2D(Main.GlobalTimeWrappedHourly * 0.187f, Main.GlobalTimeWrappedHourly * 0.193f, 2, 577215664) * 0.05f;
                    return offset;
            }
