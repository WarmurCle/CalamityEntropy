...
namespace CalamityEntropy.Content.NPCs.AbyssalWraith
{
    ...
    public class AbyssalWraith : ModNPC
    {
        ...
        public override void AI()
        {
            ...
            wingRotLeft *= 0.86f;
            wingRotRight *= 0.86f;
            if (CEUtils.getDistance(NPC.Center, Main.LocalPlayer.Center) < 8000 && !NPC.Entropy().ToFriendly && !Main.dedServ)
            {
            ...
            if (portalTime <= 0)
            {
                if (NPC.HasValidTarget)
                {
                    ...
                    else
                    {
                        ...
                        if (NPC.ai[1] == 2)
                        {
                            ...
                            if (NPC.ai[2] > 40)
                            {
                                if (NPC.ai[2] % 60 == 0)
                                {
                                    int a = (int)MathHelper.ToDegrees(CEUtils.randomRot());
                                    for (int i = 0; i < 5; i++)
                                    {
                                        if (Main.netMode != NetmodeID.MultiplayerClient)
                                        {
                    ...
                    if (counter % 10 == 0 && CEUtils.getDistance(NPC.Center, target.Center) > 4000)
                    {
