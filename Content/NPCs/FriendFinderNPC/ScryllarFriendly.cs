            ...
            if (target is NPC)
            {
                ...
                else
                {
                    ...
                    if (NPC.ai[1] < 26)
                    {
                        for (int i = 0; i < 6; i++)
                        {
                            EParticle.NewParticle(new Smoke() { timeleftmax = 30, timeLeft = 30 }, NPC.Center + Utilities.Util.randomVec(8), Utilities.Util.randomRot().ToRotationVector2() * Main.rand.NextFloat(0, 0.6f) + new Vector2(0, -1.2f), Color.OrangeRed, 0.16f, 0.14f, true, Microsoft.Xna.Framework.Graphics.BlendState.Additive);
                        }
                    }
                    if (NPC.ai[1] < 30)
                    {
