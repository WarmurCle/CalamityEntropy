    ...
    public class AzafureShieldDash : PlayerDashEffect
    {
        ...
        public override void MidDashEffects(Player player, ref float dashSpeed, ref float dashSpeedDecelerationFactor, ref float runSpeedDecelerationFactor)
        {
            ...
            else
            {
                ...
                {
                    //ChargeShield冲刺火花,同DriverCore那套CalamityPorts LineCal
                    PRTLoader.NewParticle<PRT_LineCal>(player.Center + new Vector2(0, 24), -player.velocity.RotatedBy(-0.2f * player.direction) * 0.4f, sparkColor, sparkScale).Configure(false, (int)(sparkLifetime));
                    PRTLoader.NewParticle<PRT_LineCal>(player.Center + new Vector2(0, -24), -player.velocity.RotatedBy(0.2f * player.direction) * 0.4f, sparkColor, sparkScale).Configure(false, (int)(sparkLifetime));
                }
                {
                    sparkLifetime = 12;
                    sparkColor = Color.LightGoldenrodYellow;
                    PRTLoader.NewParticle<PRT_LineCal>(player.Center + new Vector2(0, 16), -player.velocity.RotatedBy(0.2f * player.direction) * 0.4f, sparkColor, sparkScale).Configure(false, (int)(sparkLifetime));
                    PRTLoader.NewParticle<PRT_LineCal>(player.Center + new Vector2(0, -16), -player.velocity.RotatedBy(-0.2f * player.direction) * 0.4f, sparkColor, sparkScale).Configure(false, (int)(sparkLifetime));
                }
            }
