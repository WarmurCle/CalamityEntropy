...
namespace CalamityEntropy.Content.Items.Books.BookMarks
{
    ...
    public class CrimsonBMEffect : EBookProjectileEffect
    {
        ...
        {
            if (Main.rand.NextBool(projectile.hasEffect<APlusBMEffect>() ? 3 : 5))
            {
                for (int i = 0; i < 16; i++)
                {
                    Projectile.NewProjectile(projectile.GetSource_FromThis(), target.Center + new Vector2(Main.rand.NextFloat(-80, 80), 400), new Vector2(0, -18), ModContent.ProjectileType<BloodBlast>(), damageDone / 14, projectile.knockBack / 3, projectile.owner, 0, Main.rand.NextFloat(-0.1f, 0.1f)).ToProj().DamageType = projectile.DamageType;
                }
            }
