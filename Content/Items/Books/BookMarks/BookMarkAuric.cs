    ...
    public class BookMarkAuricBMEffect : EBookProjectileEffect
    {
        public override void OnHitNPC(Projectile projectile, NPC target, int damageDone)
        {
            ...
            for (int i = 0; i < (projectile.hasEffect<APlusBMEffect>() ? 3 : 5); i++)
            {
                ...
                {
                    Projectile.NewProjectile(projectile.GetSource_FromThis(), target.Center, Utilities.Util.randomRot().ToRotationVector2() * 16, ModContent.ProjectileType<ZeratosBullet0>(), projectile.damage / 5, projectile.knockBack, projectile.owner).ToProj().DamageType = projectile.DamageType;
                }
            }
        }
    }
