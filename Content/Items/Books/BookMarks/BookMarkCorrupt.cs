    ...
    public class CorruptBMEffect : EBookProjectileEffect
    {
        ...
        public override void OnHitNPC(Projectile projectile, NPC target, int damageDone)
        {
            if (Main.rand.NextBool(projectile.HasEBookEffect<APlusBMEffect>() ? 3 : 5))
            {
                ...
                Projectile.NewProjectile(projectile.GetSource_FromThis(), target.Center, shotDir, 496, (damageDone / 2).Softlimitation(125), projectile.knockBack / 3, projectile.owner, 0, Main.rand.NextFloat(-0.1f, 0.1f)).ToProj().DamageType = projectile.DamageType;
            }
