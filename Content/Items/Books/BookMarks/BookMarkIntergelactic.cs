    ...
    public class IGBMEffect : EBookProjectileEffect
    {
        public override void BookUpdate(Projectile projectile, bool s)
        {
            if (s && projectile.ModProjectile is EntropyBookHeldProjectile book)
            {
                ...
                {
                    Projectile.NewProjectile(projectile.GetSource_FromAI(), projectile.Center, projectile.velocity * 0.6f, ModContent.ProjectileType<NovaSlimerProj>(), book.CauculateProjectileDamage(1.25f), projectile.knockBack, projectile.owner);
                }
            }
        }
    }
