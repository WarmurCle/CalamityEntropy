    ...
    public class AbyssalCrack : ModProjectile
    {
        ...
        public void draw()
        {
            ...
            for (int i = 1; i < points.Count; i++)
            {
                ...
                Util.Util.drawLine(Main.spriteBatch, px, points[i - 1], points[i] + jv, color * jd, 1f * lw * (new Vector2(-30, 0).RotatedBy(MathHelper.ToRadians(180 * ((float)i / points.Count)))).Y, 3);
            }
        }
    }
