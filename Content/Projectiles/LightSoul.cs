            ...
            else
            {
                ...
                if (target != null)
                {
                    ...
                    if (Projectile.getRect().Intersects(target.getRect())) 
                    {
                        for (int i = 0; i < 16; i++)
                        {
                            EParticle.spawnNew(new GlowSpark(), Projectile.Center, Util.Util.randomRot().ToRotationVector2() * Main.rand.NextFloat(2, 7), Color.White, Main.rand.NextFloat(0.08f, 0.12f), 1, true, BlendState.Additive, 0);
                        }
                        Util.Util.PlaySound("soulexplode", 1.2f, Projectile.Center, maxIns: 2, volume: 0.6f);
                        ...
                        Projectile.Kill();
                        target.Kill();
                        Projectile.getOwner().Heal(Projectile.getOwner().statLifeMax2 / 100);
                        return;
                    }
                }
                else
        ...
        public void drawT() {
            ...
            if (mp.odp.Count > 1)
            {
                ...
                for (int i = 1; i < mp.odp.Count; i++)
                {
                    ...
                    ve.Add(new Vertex(mp.odp[i] - Main.screenPosition + (mp.odp[i] - mp.odp[i - 1]).ToRotation().ToRotationVector2().RotatedBy(MathHelper.ToRadians(90)) * 9 * Projectile.scale,
