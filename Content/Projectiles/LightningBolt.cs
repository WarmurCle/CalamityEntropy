        ...
        public override bool PreDraw(ref Color lightColor)
        {
            if (odp.Count > 1)
            {
                for (int i = 1; i < odp.Count; i++)
                {
                    ...
                    Util.Util.drawLine(Main.spriteBatch, ModContent.Request<Texture2D>("CalamityEntropy/Assets/Extra/white").Value, odp[i - 1], odp[i], Color.White, 12 * Projectile.scale, (int)(8 * Projectile.scale));
