    ...
    public class RbCircle : ModProjectile
    {
        ...
        public override void AI()
        {
            ...
            if (player.Entropy().rBadgeActive)
            {
                ...
                {
                    if (p.active && Util.Util.getDistance(p.Center, player.Center) < 360 * Projectile.scale * 1.6f && p.hostile && p.velocity.Length() > 0.02f)
                    {
                        p.velocity += (p.Center - player.Center).SafeNormalize(new Vector2(1, 0)) * p.velocity.Length() * 0.05f * ((360 * Projectile.scale * 1.6f - Util.Util.getDistance(p.Center, player.Center)) / 360 * Projectile.scale * 1.6f);
        ...
        public override bool PreDraw(ref Color lightColor)
        {
            ...
            Texture2D t1 = Util.Util.getExtraTex("sCircle");
            Texture2D t2 = Util.Util.getExtraTex("sDot");
