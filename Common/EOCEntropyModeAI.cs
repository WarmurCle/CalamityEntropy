...
{
    public static class EOCEntropyModeAI
    {
        ...
        public static bool BuffedEyeofCthulhuAI(NPC npc, Mod mod)
        {
            ...
            // Phases
            float phase2LifeRatio = 1;
            float phase3LifeRatio = masterMode ? 0.7f : 0.4f;
            float finalPhaseRevLifeRatio = masterMode ? 0.4f : 0.4f;
            float penultimatePhaseDeathLifeRatio = masterMode ? 0.3f : 0.2f;
            float finalPhaseDeathLifeRatio = masterMode ? 0.2f : 0.1f;
            bool phase2 = true;
            ...
            servantAndProjectileVelocity *= 1.2f;
            ...
            else
            {
                ...
                else if (npc.ai[1] == 5f)
                {
                    ...
                    if (npc.ai[2] > timeGateValue)
                    {
                        ...
                        hoverSpeed += velocityScalar * 0.06f;
                        hoverAcceleration += velocityScalar * 0.003f;
                    }
