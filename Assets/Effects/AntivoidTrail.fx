...
float4 EffectFunction(float2 coords : TEXCOORD0) : COLOR0
{
    ...
    return ((color2 * float4(0.2, 0.06, 0.3, color2.r + z)) + (float4(color1.r, color1.g, color1.b, color1.r) * float4(0.96, 0.96, 1, color2.r + z)) * color2.r + color3 * float4(0.3, 0.2, 0.8, 0.2 * color3.r * (color2.r + z))) * float4(1, 1, 1, alpha);
}
